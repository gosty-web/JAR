...
function Test-Args {
    ...
    if ($Version -and -not (Test-Semver $Version)) {
        Invoke-ThrowError 2 "-Version must be fully-qualified semver (e.g. 2.27.41), got: $Version"
    }
    return $true
}
...
function Assert-Platform {
    if (-not (Test-Windows)) {
        Invoke-ThrowError 1 'unsupported OS. install.ps1 supports only Windows; use install.sh on Linux/macOS.'
    }
}
...
function Assert-SystemPrivilege {
    if ($System -and -not (Test-IsAdmin)) {
        Invoke-ThrowError 1 '-System requires admin privileges. Re-run from an elevated PowerShell, or omit -System for a user-local install.'
    }
}
...
function Resolve-InstallPath {
    if ($System) {
        ...
        if (-not $env:LOCALAPPDATA) {
            Invoke-ThrowError 2 'LOCALAPPDATA is not set; cannot resolve user-local install path.'
        }
...
function Receive-WithRetry {
    ...
    try {
        ...
        Invoke-ThrowError 1 "failed to download $Label from $Url after one retry: $($_.Exception.Message)"
    }
}
...
function Receive-Installer {
    ...
    Receive-WithRetry -Url (Get-InstallerUrl) -Out $Script:InstallerPath -Label 'AWS CLI installer'
}
...
function Test-Installer {
    ...
    if ($sig.Status -ne 'Valid') {
        Invoke-ThrowError 1 ("Authenticode signature verification failed for " +
...
function Save-PreInstallVersion {
...
function Start-Installer {
    ...
    if ($MsiexecSuccessCodes -notcontains $proc.ExitCode) {
        Invoke-ThrowError 1 "msiexec failed with exit code $($proc.ExitCode)"
    }
}
...
# errors are printed here -- both Invoke-ThrowError (which sets $Script:ExitCode)
